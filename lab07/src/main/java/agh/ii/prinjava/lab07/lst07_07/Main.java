...
public class Main {
    ...
    private static void demo4() {
        ...
        final Object[] theLongestMovies = optMovies.orElseThrow().stream()
                .filter(m -> m.runtime() > 180) // gives out a stream with only film longer than 180 min
                .sorted((m1, m2) -> Integer.compare(m2.runtime(), m1.runtime())) // sort from longest to shorter
                .toArray();
