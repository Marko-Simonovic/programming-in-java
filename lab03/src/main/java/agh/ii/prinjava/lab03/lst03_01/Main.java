 ...
 * <br><br>
 * @see <a href="https://www.youtube.com/watch?v=JTjeGpSUL2M&ab_channel=MikeDane">ALL EXPLICATION</a>
 * <br>
 * In resume, <b>'Exception' catches ALL exceptions thrown while Error catches all the error.<br>
 * We should try to catch exception since they are just small abnormal things. However,
 * a program should not try to catch Error since they are serious problems that we don't
 * want to have.<br>
 * Finally, 'Throwable' catches all Exceptions AND Error that are thrown</b>
 */
public class Main {
    /**
     ...
     * <br><br><b>Here's the bottom line guideline:
     ...
     * make it an unchecked exception</b>
     */
     ...
     * (throws/try-catch are not "forced" by the compiler)<br>
     * <b>Generally RuntimeExceptions are exceptions that can be prevented programmatically.
     ...
     * Similarly ArrayIndexOutOfBoundException would never occur if you check the index first.</b>
     * <br>
     * All the exceptions which derive from RuntimeException are referred to as unchecked
     * exceptions. And all the other exceptions are checked exceptions.
     * A checked exception must be caught somewhere in your code, otherwise, it will not compile.
     * That is why they are called checked exceptions. On the other hand, with
     * <b>unchecked exceptions, the calling method is under no obligation to handle or declare it.</b>
     */
