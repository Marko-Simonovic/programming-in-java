...
public class Main {
     ...
     * <br><br><b>If execution of the try block completes normally,
     * then the finally block is executed, and then there is a choice:<br>
     * If the finally block completes normally, then the try statement completes normally.<br>
     * If the finally block completes abruptly for reason S, then the try statement
     * completes abruptly for reason S.</b><br>
     * TO RESUME, each time the finally block ends abruptly, the 'try' block will not end.
     */
    ...
    /**
     * return + try-finally block (hint: use the debugger to see that
     * <b>the finally-block is executed as the last) and not the return call.</b>
     */
