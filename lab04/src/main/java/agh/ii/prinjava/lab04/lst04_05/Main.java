...
/**
 * A <b>bounded type variable</b> is a variable which type is restricted.<br>
 * When we define the generic class we can specify a specific type that we would like to have.
 * For example, if we have a GenBase<T> in which I can send Integer just like String and Character.
 * I can specify <i>GenBase(T<b> extends Number</b>)</i> so we can only pass variables that are Numbers.
 */
class GenBase<T> {
    private T x;
...
public class Main {
    ...
    /*interface Moveable<T> {
        void moveAll(T[] elems, double x, double y);
    }
    private static <T> implements Moveable void moveAll(T[] elems, double x, double y) {
        for (var e : elems) {
            e.goTo(x, y);
        }
    }*/
